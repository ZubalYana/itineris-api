...
export const inviteRepository = {
  async create(
    ...
    return await prisma.invite.create({
      data: { ...data, tripId, invitedById: userId, token, expiresAt },
    });
  },
  ...
  async findById(id: string) {
    return await prisma.invite.findUnique({ where: { id } });
  },

  async cancel(id: string) {
    return await prisma.invite.delete({ where: { id } });
  },
  ...
  async accept(inviteTripId: string, userId: string, inviteId: string) {
    return prisma.$transaction([
      prisma.tripMember.create({
        data: { tripId: inviteTripId, userId, role: "COLLABORATOR" },
      }),
